...
const processes = [{
  ...
  steps: ["Initial consultation and requirement gathering", "Concept development and refinement", "Rapid prototyping and early testing", "Feasibility assessment and component selection"]
}, {
  id: 2,
  ...
  steps: ["Sensor and hardware customization", "Embedded software development", "Power management optimization", "Cloud platform integration and security implementation"]
}, {
  id: 3,
  ...
  steps: ["Algorithm development and training", "Data pipeline establishment", "Analytics dashboard creation", "Machine learning model optimization"]
}, {
  id: 4,
  ...
  steps: ["Manufacturing partner selection", "Quality assurance processes", "Regulatory compliance and certification", "Initial production run supervision"]
}, {
  id: 5,
  ...
  steps: ["Go-to-market strategy refinement", "Production scaling planning", "Distribution channel establishment", "Continuous improvement processes"]
}];
const Process = () => {
  ...
  return <section id="process" className="bg-white relative overflow-hidden py-[25px]">
          ...
          <div className="space-y-10 relative">
            {processes.map((process, index) => <div key={process.id} ref={el => processSectionsRef.current[index] = el} className={cn("relative flex flex-col md:flex-row md:items-center gap-6", index % 2 === 0 ? "md:flex-row" : "md:flex-row-reverse text-right")}>
                <div className="md:w-1/2">
                  <div className={cn("md:absolute top-0 left-1/2 md:-translate-x-1/2 w-12 h-12 rounded-full flex items-center justify-center z-10 transition-all duration-300", activeProcess === process.id ? "bg-wrlds-teal text-white" : "bg-white text-wrlds-teal border border-wrlds-teal/30")} onClick={() => setActiveProcess(process.id)}>
